...
class StaticSiteTests(unittest.TestCase):
    ...
    def test_all_configured_datasets_exist(self):
        source = (ROOT / "js" / "map.js").read_text(encoding="utf-8")
        paths = set(
            re.findall(
                r"""(?m)^\s*dataFile\s*=\s*['"](data/[^'"]+\.json)""",
                source,
            )
        )
    ...
    def test_builder_is_clean_and_repeatable(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            for excluded in ("src", "tests", "scripts", ".git", "pyproject.toml"):
                self.assertFalse((output / excluded).exists())

            (output / "obsolete.txt").write_text("stale", encoding="utf-8")
    ...
    def test_builder_refuses_unmanaged_directories(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(original.read_text(encoding="utf-8"), "Do not delete")
    ...
    def check_http_site(self, directory, prefix):
        ...
        try:
            ...
            self.assertEqual(html, fetch("index.html").decode("utf-8"))
